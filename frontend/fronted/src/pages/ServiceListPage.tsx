...
const ServiceListPage: React.FC = () => {
  ...
  const fetchServices = async (term = "") => {
    ...
    try {
      const res = await fetch(
        `${apiUrl}/service?search=${encodeURIComponent(term)}`
      );
  ...
  const handlelinkClick = (e: React.MouseEvent, _serviceId?: string) => {
    ...
    if (!token) {
      e.preventDefault();
      setMessage(
        "Vänligen logga in eller skapa ett konto för att boka en tid online."
      );
  ...
  return (
    <div className="service-page">
      ...
      <p className="info-box">
        Alla priser gäller för tovfri, lätthanterlig hund eller katt. Vid
        tovor/tovutredning kan extra kostnad tillkomma.
      </p>
        ...
        <>
          <div className="service-section">
            ...
            <div className="dog-size-info">
              <p>
                <strong>Liten hund:</strong> upp till 10 kg
              </p>
              <p>
                <strong>Mellan hund:</strong>10-25 kg
              </p>
              <p>
                <strong>Stror hund:</strong>över 25 kg
              </p>
            </div>
            <div className="service-grid">
              {dogServices.map((service) => (
                <div key={service._id} className="service-card">
                  ...
                  <p>
                    <strong>Tid:</strong> {service.duration} min
                  </p>
                  <Link
